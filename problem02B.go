 ...
 * Toboggan Corporate Policies have no concept of "index zero"!) Exactly one of these
 ...
 */
 ...
 func problem02B(fileName string) int {
	...
	occur := 0
	 ...
	 for scanner.Scan() {
		 passwordRow := scanner.Text()
		 pos1S, b := splitInTwoAt(passwordRow, "-")
		 pos1, err := strconv.Atoi(pos1S)
		 if err != nil {
			 log.Fatal(err)
		 }
		 pos2S, c := splitInTwoAt(b, " ")
		 pos2, err := strconv.Atoi(pos2S)
		 if err != nil {
			log.Fatal(err)
		}
		chr, d := splitInTwoAt(c, ":")
		pass := strings.Trim(d, " ")
		if (pass[pos1-1] == chr[0] && pass[pos2-1] != chr[0]) || (pass[pos1-1] != chr[0] && pass[pos2-1] == chr[0]) {
			 occur++
		}
