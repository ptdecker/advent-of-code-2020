...
func splitInTwoAt(s string, delimiter string) (string, string) {
 ...
 func problem02A(fileName string) int {
	...
	occur := 0
	 ...
	 for scanner.Scan() {
		 ...
		 minS, b := splitInTwoAt(passwordRow, "-")
		 min, err := strconv.Atoi(minS)
		 if err != nil {
			 log.Fatal(err)
		 }
		 maxS, c := splitInTwoAt(b, " ")
		...
		chr, d := splitInTwoAt(c, ":")
		 pass := strings.Trim(d, " ")
		 chrCnt := strings.Count(pass, chr)
		 if (chrCnt >= min) && (chrCnt <= max) {
			 occur++
		 }
